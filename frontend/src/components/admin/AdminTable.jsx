...
const AdminTable = ({ title, columns, data, onDelete, onEdit }) => {
  return (
    <div className="mb-6 sm:mb-10">
      <h2 className="font-semibold text-base sm:text-lg mb-2 text-blue-600">{title}</h2>
      <div className="overflow-x-auto rounded-xl border border-gray-100 bg-white shadow">
        <table className="min-w-[600px] sm:min-w-full divide-y divide-gray-200 text-xs sm:text-sm">
          <thead className="bg-blue-50">
            <tr>
              {columns.map(col => (
                <th key={col.key} className="px-2 sm:px-4 py-2 text-left font-bold text-blue-700 uppercase tracking-wider whitespace-nowrap">{col.label}</th>
              ))}
              <th className="px-2 sm:px-4 py-2 text-left font-bold text-blue-700 uppercase tracking-wider whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-100">
            {data.map(row => (
              <tr key={row._id}>
                {columns.map(col => (
                  <td key={col.key} className="px-2 sm:px-4 py-2 text-gray-700 whitespace-nowrap">{row[col.key]}</td>
                ))}
